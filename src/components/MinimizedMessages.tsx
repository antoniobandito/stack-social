...
const MinimizedMessages: React.FC<MinimizedMessagesProps> = ({
  ...
  const toggleExpanded = () => {
    if (isExpanded) {
      setActiveConversationId(null); // reset when minimizing
    }
  ...
  return (
    ...
    >
      {/* Header */}
      <div className={`flex items-center justify-between p-2 h-10 text-black ${isExpanded ? 'border-b' : ''}`}>
        ...
        <div className="flex items-center space-x-1">
          <button onClick={toggleExpanded} className="p-1 hover:bg-gray-200 rounded text-black">
            {isExpanded ? <IoMdArrowDropdownCircle size={16} /> : <IoMdArrowDropupCircle size={16} />}
          </button>
          <button onClick={expandToFullMessages} className="p-1 hover:bg-gray-200 rounded text-black">
            <IoMdExpand size={16} />
          </button>
          <button onClick={handleClose} className="p-1 hover:bg-gray-200 rounded text-black">
            <IoMdClose size={16} />
          </button>
        </div>
      </div>
      ...
      <div
        className={`flex flex-col transition-all duration-300 overflow-hidden ${
          isExpanded ? 'h-[calc(100%-40px)]' : 'h-0'
        }`}
      >
        {isExpanded && activeConversationId ? (
          <div className="flex-1 overflow-hidden">
            <MessageThread conversationId={activeConversationId} key={activeConversationId} />
          </div>
        ) : isExpanded ? (
                    ...
                    <div className="flex justify-between items-baseline">
                      ...
                      >
                        {otherUserIds
                          .map(
                            (id) =>
                              userProfiles[id]?.username || `User ${id.substring(0, 4)}`
                          )
                          .join(', ')}
                      </div>
                      {hasUnread && (
